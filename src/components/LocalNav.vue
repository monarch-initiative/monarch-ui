<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-for="(row, rowIndex) in classRows" :key="rowIndex" class="row">
        <div class="col-4">
          <button
            v-if="row.superclass"
        ...
        <div class="col-4">
          <button
            ...
            class="m-1 btn btn-sm btn-info"
            @click="
              emitSelection(row.equivalentClass.label, row.equivalentClass.id)
            "
          >
            {{ row.equivalentClass.label }} <br />
            ({{ row.equivalentClass.id }})
          </button>
        </div>
...
export default {
  ...
  methods: {
    async getCurieRelationships() {
      const that = this;
      try {
        const searchResponse = await BL.getNodeSummary(
          this.anchorId,
          this.anchorType
        );
