<template>
  <div class="assoc-table">
    ...
    <div v-show="!dataError && initialLoad">
      ...
      <div>
        <h5>
          <strong v-if="cardType === 'phenotype' && totalAssociations >= 0">{{
            totalAssociations
          }}</strong>
          <strong v-else-if="totalAssociations > 0">{{
            totalAssociations
          }}</strong>
      ...
      <div v-if="cardType === 'phenotype' && totalAssociations == 0">
        <template v-if="nodeType === 'gene'">
          We are not aware of any authoritative sources of observed human
          phenotypes
          <span v-if="altDisplayGene == 0"> or diseases </span>
          for this gene.
          <span v-if="altDisplayGene > 0">
            However, we encourage you to view the available
          </span>
          <span v-if="altDisplayGene == 1">
            <a :href="this.$route.path + '#ortholog-phenotype'"
              >{{ cardCounts["ortholog-phenotype"] }} ortholog phenotype(s)</a
            >
            in other species.
          </span>
          <span v-if="altDisplayGene == 2">
            <a :href="this.$route.path + '#causal-disease'"
              >{{ cardCounts["causal-disease"] }} associated disease(s)</a
            >.
          </span>
          <span v-if="altDisplayGene == 3">
            <a :href="this.$route.path + '#correlated-disease'"
              >{{ cardCounts["correlated-disease"] }} associated disease(s)</a
            >.
          </span>
          <span v-if="altDisplayGene == 4">
            <a :href="this.$route.path + '#ortholog-disease'"
              >{{ cardCounts["ortholog-disease"] }} associated disease(s)</a
            >.
          </span>
        </template>
        <template v-else-if="nodeType === 'disease'">
          We are not aware of any publicly available source data for phenotypes
          <span v-if="altDisplayDisease == 0"> or genes </span>
          for this disease.
          <span v-if="altDisplayDisease > 0">
            However, we encourage you to view the available
          </span>
          <span v-if="altDisplayDisease == 1">
            <a :href="this.$route.path + '#causal-gene'"
              >{{ cardCounts["causal-gene"] }} causal gene(s)</a
            >.
          </span>
          <span v-if="altDisplayDisease == 2">
            <a :href="this.$route.path + '#correlated-gene'"
              >{{ cardCounts["correlated-gene"] }} correlated gene(s)</a
            >.
          </span>
          <span v-if="altDisplayDisease == 3">
            <a :href="this.$route.path + '#gene'"
              >{{ cardCounts["gene"] }} associated gene(s)</a
            >.
          </span>
        </template>
        <template v-else-if="nodeType === 'variant'">
          We are not aware of any phenotypes
          <span v-if="cardCounts['disease'] == 0"> or diseases </span>
          specifically associated with this variant.
          <span v-if="cardCounts['disease'] > 0">
            However, we encourage you to view the available
            <a :href="this.$route.path + '#disease'"
              >{{ cardCounts["disease"] }} associated disease(s)</a
            >.
          </span>
        </template>
    ...
    async fetchData(reset) {
      ...
      try {
        ...
        if (this.cardType === "publication") {
          const ids = that.associationData.associations.map(
            ({ object }) => object.id
          );
    ...
    populateRows() {
      ...
      this.associationData.associations.forEach((elem) => {
        ...
        if (modifiedCardType === "publication") {
          objectElem.label =
            truncate(elem.publicationMeta?.title, 50) || objectElem.label;
        }
