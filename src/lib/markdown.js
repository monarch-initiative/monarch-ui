...
function applyLinkHandlersFunction(parser) {
  ...
  const defaultRenderLinkOpen =
    ...
    function defaultRenderLinkOpen(tokens, idx, options, env, self) {
      return self.renderToken(tokens, idx, options);
    };
  const defaultRenderLinkClose =
    parser.renderer.rules.link_close ||
    function defaultRenderLinkClose(tokens, idx, options, env, self) {
      return self.renderToken(tokens, idx, options);
    };

  let inRouterLink = false;

  parser.renderer.rules.link_open = function link(
    tokens,
    idx,
    options,
    env,
...
`;
  ...
  parser.renderer.rules.link_close = function link(
    tokens,
    idx,
    options,
    env,
  ...
  parser.renderer.rules.blockquote_open = function link() {
