...
const predicates = {
  // predicates used to retrieve items from BBOP graph json emitted by biolink
...
const curiePrefixURLs = {
  // various curie prefixes that need to be fixed/expanded to URLs
...
export function _subjectPredicate2Object(subjectIRI, predicate, graph) {
  return us
    .chain(_subjectPredicate2Objects(subjectIRI, predicate, graph))
    .first()
    .value();
}
...
export function mergeStaticData(sourceData, staticSourceData) {
  ...
  us.each(staticSourceData, function fn(staticDatum) {
    if (!dynamicSummaryIRIs.includes(staticDatum.summaryIRI)) {
      const newSourceEntry = populateSourceTemplate({
        _summary_iri: staticDatum.summaryIRI,
      });
...
export function populateSourceFiles(sourceData, graph) {
  for (let i = 0; i < sourceData.length; i++) {
    const sources = _subjectPredicate2Objects(
      sourceData[i]._version_iri,
      predicates.source,
      graph
    );
    sourceData[i].sourceFiles = us
      .chain(sources)
...
export function populateIngestDate(sourceData, graph) {
  for (let i = 0; i < sourceData.length; i++) {
    sourceData[i].ingestDate = graph.get_node(
      sourceData[i]._version_iri
    )._metadata[predicates.created][0];
  }
}

export function populateRdfDownloadUrl(sourceData, graph) {
  for (let i = 0; i < sourceData.length; i++) {
    const distributionIRI = _versionIRI2distributionIRI(
      sourceData[i]._version_iri,
      graph
    );
    const downloadUrl = _subjectPredicate2Object(
      distributionIRI,
      predicates.downloadUrl,
      graph
    );
    if (downloadUrl !== undefined) {
      sourceData[i].rdfDownloadUrl = downloadUrl.replace(
        "MonarchArchive:",
        curiePrefixURLs.MonarchArchive
      );
    }
  }
}
